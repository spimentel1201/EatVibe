...
import com.foodrush.payment.application.usecase.RefundPaymentUseCase;
...
public class PaymentController {
        ...
        @PostMapping("/refund")
        @Operation(summary = "Procesar un reembolso", description = "Procesa un reembolso total o parcial de una transacción completada")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Reembolso procesado exitosamente"),
                        @ApiResponse(responseCode = "400", description = "Datos de reembolso inválidos o transacción no válida", content = @Content(schema = @Schema(implementation = ProblemDetail.class))),
                        @ApiResponse(responseCode = "404", description = "Transacción no encontrada", content = @Content(schema = @Schema(implementation = ProblemDetail.class)))
        })
        public ResponseEntity<Void> processRefund(
                        @Valid @RequestBody RefundRequest request) {
                log.info("POST /api/v1/payments/refund - Transaction: {}, Amount: {}",
                                request.transactionId(), request.amount());

                refundPaymentUseCase.execute(request);
